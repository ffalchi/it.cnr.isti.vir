...
public class HammingLongs  implements IMetric<BinaryLongs> {
	...
	public final double distance(BinaryLongs f1, BinaryLongs f2) {
		return Hamming.distance_norm(f1.getValues(), f2.getValues() );	
	}
	
	@Override
	public final double distance(BinaryLongs f1, BinaryLongs f2, double max) {
		return Hamming.distance_norm(f1.getValues(), f2.getValues(), max );
	}
